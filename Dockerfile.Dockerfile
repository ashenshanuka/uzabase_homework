...
# Create a new conda environment with Python 3.11 and install packages.
RUN conda create -n myenv python=3.11 -y && \
    conda run -n myenv conda install -c conda-forge \
        pyspark \
        pytorch \
        numpy \
        pandas \
        ...
        sqlalchemy \
        pytest -y && \
    conda run -n myenv pip install neo4j && \
    conda clean -afy

# Update PATH so the 'myenv' environment is the default for python/pip
